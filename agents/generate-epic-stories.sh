...
while [[ $# -gt 0 ]]; do
	...
	*)
		...
		echo "Usage: $0 [-i|--interactive] [--brd-path PATH] [--existing-app-brd PATH] [--existing-app-arch PATH] [--workspace-root PATH] [--context-dirs DIRS] [--output-dir DIR] [--policy-file FILE]"
		exit 1
		;;
	esac
done
...
if [ "$INTERACTIVE_MODE" = true ]; then
	...
	read -p "Enter output directory [default: sdlc-artifacts]: " input
	OUTPUT_DIR="${input:-sdlc-artifacts}"
...
if [ "$INTERACTIVE_MODE" = false ]; then
	...
	OUTPUT_DIR="${OUTPUT_DIR:-${ENV_OUTPUT_DIR:-sdlc-artifacts}}"
...
mkdir -p "$OUTPUT_DIR/epic-stories"
...
- Output Directory: $OUTPUT_DIR/epic-stories/
...
if [ ! -f "$OUTPUT_DIR/epic-stories/epic.md" ] || [ ! -f "$OUTPUT_DIR/epic-stories/stories.md" ]; then
...
echo "=========================================="
echo "Output location: $OUTPUT_DIR/epic-stories/"
echo ""

# Display the generated files
if [ -f "$OUTPUT_DIR/epic-stories/epic.md" ]; then
	echo "Epic file: $OUTPUT_DIR/epic-stories/epic.md"
fi

if [ -f "$OUTPUT_DIR/epic-stories/stories.md" ]; then
	echo "Stories file: $OUTPUT_DIR/epic-stories/stories.md"
fi

echo ""
echo "Please review the generated epic and stories at: $OUTPUT_DIR/epic-stories/"
echo ""
...
if [ -f "$OUTPUT_DIR/epic-stories/epic.md" ]; then
	EPIC_TITLE=$(grep -m 1 "^# " "$OUTPUT_DIR/epic-stories/epic.md" | sed 's/^# //')

	# Extract just the summary/description section (first 500 chars to avoid JSON issues)
	EPIC_DESCRIPTION=$(sed -n '/## Summary/,/## /p' "$OUTPUT_DIR/epic-stories/epic.md" | head -20 | tr '\n' ' ' | cut -c1-500)

	# If no summary found, use first few lines
	if [ -z "$EPIC_DESCRIPTION" ]; then
		EPIC_DESCRIPTION=$(head -10 "$OUTPUT_DIR/epic-stories/epic.md" | tr '\n' ' ' | cut -c1-500)
	fi
else
...
if [ -f "$OUTPUT_DIR/epic-stories/stories.md" ]; then
	...
	done <"$OUTPUT_DIR/epic-stories/stories.md"
...
if [ -n "$GITHUB_OUTPUT" ]; then
	...
	echo "artifact_path=$OUTPUT_DIR/epic-stories" >>$GITHUB_OUTPUT
fi
